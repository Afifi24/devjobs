...
const Search = ({isdark}) => {
  ...
  return (
    <div className={` ${isdark ? 'bg-[#19202D] text-white duration-200 ':'bg-[#FFFFFF] duration-200 '}  relative flex justify-center items-center    -mt-10 rounded-md  min-h-[5rem]`}>
        <div className={`h-[4rem]  flex items-center justify-start px-4 gap-2 md:px-8 flex-[1.5]`}>
          ...
          <input type="text" className={`p-3 outline-none bg-transparent w-full `} placeholder='Filter by title...' />
          ...
          <FaFilter onClick={()=>setHideshow(!hideshow)} className={`text-4xl ${isdark ? 'text-white':''} text-gray-400`} />
                 <div className='bg-[#5964E0]  p-4 rounded-md '>
        ...
        <div className='h-[4rem] hidden md:flex items-center justify-between  px-8 flex-[1.5]'>
          ...
          <input type="checkbox" className='h-5 border-none bg-transparent w-5 outline-none ' />
         ...
         {
           hideshow &&
          <Popup isdark={isdark}  hideshow={hideshow} setHideshow={setHideshow}/>}
    </div>
  )
}
